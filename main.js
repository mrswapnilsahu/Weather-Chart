            ...
            await fetch('https://data.cityofchicago.org/resource/k7hf-8y75.json')
                ...
                .then(data => {
                    for (i = 0; i < data.length; i++) {
                        ...
                        var diff_time = Math.abs(curr_date.getTime() - data_date.getTime()) / (1000 * 3600);
                        if (data[i].station_name === "Foster Weather Station" && diff_time <= 6) {
                            ...
                            time.push(new Date(data[i].measurement_timestamp).toString().slice(4, 21));
                        } else if (data[i].station_name === "Oak Street Weather Station" && diff_time <= 6) {
